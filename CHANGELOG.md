...
### Added
- A composite GitHub Action for checksum-verified installation and SQL Server
  contract validation on Linux runners.
- Checksum-verified, user-local PowerShell and Bash installers with explicit or
  latest-version selection and atomic executable replacement.
- `sqlguard init` for generating a safe starter specification and exact next
  steps without assuming an application schema.
